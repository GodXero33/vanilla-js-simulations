...
const pendulum = new DoublePendulum(300, 200);
const gui = new dat.GUI();

const pendulumFolder = gui.addFolder('Pendulum Controls');

pendulumFolder.add(pendulum, 'length1', 100, 1000).onChange(val => {
	pendulum.length1 = val;

	pendulum.update();
});
pendulumFolder.add(pendulum, 'length2', 100, 1000).onChange(val => {
	pendulum.length2 = val;

	pendulum.update();
});
pendulumFolder.add(pendulum, 'gravity', 0.1, 20).step(0.1);
pendulumFolder.add(pendulum, 'frictionFact1', 0, 0.1).step(0.001);
pendulumFolder.add(pendulum, 'frictionFact2', 0, 0.1).step(0.001);
pendulumFolder.add(pendulum, 'mass1', 1, 100).step(1);
pendulumFolder.add(pendulum, 'mass2', 1, 100).step(1);
